...
4. **意味ごとにコミットを分割** - 複数の独立した変更がある場合は、意味ごとにコミットを分ける
   - 悪い例: treesitter修正とLSP設定変更を1つのコミットに
   - 良い例: treesitter修正とLSP設定変更を別々のコミットに

5. **コミット前に最新状態を確認** - コミット前に `git status` と `git diff` を実行して最新の変更を確認する（古いスナップショット情報に依存しない）
