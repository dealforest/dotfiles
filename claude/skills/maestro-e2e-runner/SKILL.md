---
name: maestro-e2e-runner
description: mood-board プロジェクトの Maestro E2E テストを実行する。ビルド、シミュレータ起動、Firebase Emulator ready 確認、ダイアログ処理、フロー単位実行（FAIL 時は画面確認 → helper/環境側のみ修正 → 再実行のループ）、結果サマリを一貫して行う。「Maestro テスト実行」「E2E テストを回して」「Maestro を動かして」「テストを走らせて」と言及された時に使用。モバイルアプリのテストや動作確認にも使用。
context: fork
agent: general-purpose
allowed-tools: Bash(xcrun:*), Bash(npx:*), Bash(pgrep:*), Bash(pkill:*), Bash(kill:*), Bash(date:*), Bash(mkdir:*), Bash(cp:*), Bash(lsof:*), Bash(curl:*), Bash(firebase:*), Bash(tail:*)
---
