function herdr-claude-layout -d "Create dev layout with tig and Claude in herdr"
    ...
    if test "$use_current_tab" = "true"
        ...
        # Create a new tab for the claude layout
        set -l tab_result ($herdr_bin tab create --workspace $workspace_id --label "claude" 2>&1)
    ...
    # Split layout:
    # 1. root pane -> down (bottom pane for claude)
    ...
    $herdr_bin pane run $pane_down "$claude_cmd"
