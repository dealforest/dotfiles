---
description: Conventional Commits形式でgitコミットを作成する
agent: commit-maker
subtask: true
---

変更を分析し、意味のある最小単位で分割して Conventional Commits 形式でコミットを作成する。

## ワークフロー

1. **ブランチ確認**: `git branch --show-current` で現在のブランチを確認
   - **main または master の場合**: 先に `git checkout -b feature/<内容に応じた名前>` でブランチを作成してから作業を進める
   - **既に feature ブランチの場合**: そのまま続行
2. `git diff` で変更内容を確認
3. **各 hunk（変更ブロック）を分析し、それぞれの「動作・振る舞い」を特定**
4. **動作ごとにグループ化**（「〜する」で説明できる単位）
5. 各グループに Conventional Commits タイプを決定
6. **`git add -p` で hunk 単位でステージし、グループごとにコミット**
7. 全グループをコミットし終えたらサマリーを表示
